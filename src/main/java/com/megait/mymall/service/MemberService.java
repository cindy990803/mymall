...
//        memberRepository.save(member);
    ...
    /**
     * 클라이언트가 아이디(username)와 비밀번호(password)를 로그인페이지에 입력함.
     * 스프링 시큐리티는 우리가 만들어 둔 UserDetailsService 빈의 loadUserByUsername()을 호출함.
     * loadUserByUsername() ~> 시큐리티가 '클라이언트로부터 받은 아이디, 비번'이 맞는지 대조할 수 있도록
     *             (+ 만료되었는지, 비밀번호 만료되었는지, 사용 가능한 계정인지...)
     *             유저 정보를 UserDetail형 객체에 담아서 보내줘야 함.
     * 스트링 시큐리티는 반환받은 UserDetails 객체를 가지고 클라이언트가 입력한 아이디/비번과 대조함.
     *
     * @param username 로그인 처리를 할 username(아이디)
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        Member member = optional.orElseThrow(() -> new UsernameNotFoundException("미등록 계정"));


//        MemberType memberType = member.getMemberType();
//
//        String type = memberType.name();  //"ROLE_ADMIN" / "ROLE_USER"
//
...
//        set.add(authority);
        ...
        //authority : 인가(권한). 인증 뒤에 체크하는 것. -> 'ROLE_'로 시작하는 문자열. ~> GrantedAuthority 인터페이스

        //원래는 UerDetails형으로 리턴했었는데... getMember()를 쓰기 위해, 필드에 Member를 가진 클래스 MemberUser를 만들었다!!
    ...
    public void login(Member member) {
        ...
        //인증 토큰을 Security 컨텍스트에 저장하자 => 로그인 되었다는 뜻
